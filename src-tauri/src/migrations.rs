...
mod tests {
    ...
    #[test]
    fn test_apply_migrations_creates_tables_and_seeds_admin() {
        ...
        let admin_username: String = conn
            .query_row(
                "SELECT username FROM users WHERE username = 'admin' AND role = 'Admin'",
                [],
                |row| row.get(0),
            )
            .expect("admin user not found in seeded db");
        assert_eq!(admin_username, "admin");

        // Verify idempotency
        apply_migrations(&conn).expect("second run of migrations failed");
    }
}
